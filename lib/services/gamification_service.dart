  ...
  Future<void> onSesionCompletada(String uid, DateTime ahoraLocal,
    ...
    if ([7, 14, 30].contains(nuevaRacha)) {
      final badge = nuevaRacha == 7
          ...
          : nuevaRacha == 14
              ? 'assets/badges/racha_plata.png'
              : 'assets/badges/racha_oro.png';
      ...
      final logro = Logro(
        id: logroId,
        nombre:
            'Racha ${nuevaRacha == 7 ? "Bronce" : nuevaRacha == 14 ? "Plata" : "Imparable"}',
...
// Constancia semanal (ejemplo: 4 días por semana durante 3 semanas seguidas)
    ...
    if (semanasConstantes >= 3 &&
        !(await repo.existeLogro(uid, 'constancia:3'))) {
      await repo.otorgarLogro(
          uid,
          Logro(
            id: 'constancia:3',
            ...
            descripcion:
                'Entrenaste al menos 4 días por semana durante 3 semanas seguidas.',
            puntosOtorgados: 100,
            tipo: 'frecuencia',
            periodo: null,
            otorgadoEn: ahoraLocal,
...
// Mes sin faltar
    ...
    if (await mesSinFaltar(uid, mesActual) &&
        !(await repo.existeLogro(uid, 'mesSinFaltar:${keyMes(ahoraLocal)}'))) {
      await repo.otorgarLogro(
          uid,
          Logro(
...
// Especialista en ejercicio (ejemplo: sentadillas)
    ...
    if (sentadillas >= 1000 &&
        !(await repo.existeLogro(uid, 'especialista:sentadillas'))) {
      await repo.otorgarLogro(
          uid,
          Logro(
...
// PR alcanzado (ejemplo: sentadillas)
    ...
    if (prSentadillas >= 100 &&
        !(await repo.existeLogro(uid, 'pr:sentadillas'))) {
      await repo.otorgarLogro(
          uid,
          Logro(
            id: 'pr:sentadillas',
            ...
            descripcion:
                '¡Nuevo récord de peso en sentadillas: $prSentadillas kg!',
            puntosOtorgados: 100,
            tipo: 'pr',
            periodo: null,
            otorgadoEn: ahoraLocal,
...
// Rutina variada
    if (await rutinaVariada(uid, ahoraLocal) &&
        !(await repo.existeLogro(
            uid, 'rutinaVariada:${keySemana(ahoraLocal)}'))) {
      await repo.otorgarLogro(
          uid,
          Logro(
            ...
            descripcion:
                'Entrenaste todos los grupos musculares principales esta semana.',
            puntosOtorgados: 100,
            tipo: 'variedad',
...
// Explorador de ejercicios
    ...
    for (final meta in [10, 20, 50]) {
      ...
      if (ejerciciosDistintos >= meta &&
          !(await repo.existeLogro(uid, logroId))) {
        await repo.otorgarLogro(
            uid,
            Logro(
              id: logroId,
...
// 100% cumplido (para la sesión actual)
    if (await sesionCumplida(sesionId) &&
        !(await repo.existeLogro(uid, 'cumplido:$sesionId'))) {
      await repo.otorgarLogro(
          uid,
          Logro(
            ...
            nombre: '100% cumplido',
            descripcion:
                'Completaste todas las repeticiones y ejercicios planificados en esta sesión.',
            puntosOtorgados: 50,
            tipo: 'cumplimiento',
            periodo: keyDia(ahoraLocal),
            otorgadoEn: ahoraLocal,
...
// Semana perfecta
    if (await semanaPerfecta(uid, ahoraLocal) &&
        !(await repo.existeLogro(
            uid, 'semanaPerfecta:${keySemana(ahoraLocal)}'))) {
      await repo.otorgarLogro(
          uid,
          Logro(
            ...
            descripcion:
                'Cumpliste el 100% de las sesiones y ejercicios planificados esta semana.',
            puntosOtorgados: 150,
            tipo: 'cumplimiento',
            periodo: keySemana(ahoraLocal),
            otorgadoEn: ahoraLocal,
...
// Constancia semanal: entrenar al menos X días cada semana durante N semanas seguidas
  Future<int> contarSemanasConstantes(
      String uid, int diasPorSemana, int semanasMin) async {
  ...
  Future<double> prEjercicio(String uid, String ejercicio) async {
    ...
    for (var doc in snap.docs) {
      ...
      for (var e in ejercicios) {
        if ((e['nombre'] ?? '') == ejercicio) {
          final peso =
              (e['peso_usado'] ?? e['pesoPlanificado'])?.toDouble() ?? 0;
          if (peso > pr) pr = peso;
        }
      }
    }
