      ...
      body: user == null
          ...
          : FutureBuilder<DocumentSnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                final disponibilidadSemanal = userData?['disponibilidadSemanal']?.toString() ?? '';
                ...
                if (!modoEdicion) {
                  ...
                  if (userData != null && objetivos.contains(userData['objetivo'])) {
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (modoEdicion) ...[
                          ...
                          TextFormField(controller: apellidoController, decoration: const InputDecoration(labelText: 'Apellido')),
                        ] else ...[
                          Text('Nombre y Apellido', style: Theme.of(context).textTheme.labelLarge),
                        ...
                        const SizedBox(height: 16),
                        if (modoEdicion)
                          TextFormField(controller: pesoController, decoration: const InputDecoration(labelText: 'Peso (kg)'))
                        else ...[
                          Text('Peso (kg)', style: Theme.of(context).textTheme.labelLarge),
                          const SizedBox(height: 4),
                          Text('$peso kg'),
                        ],
                        const SizedBox(height: 16),
                        if (modoEdicion)
                          TextFormField(controller: alturaController, decoration: const InputDecoration(labelText: 'Altura (cm)'))
                        else ...[
                          Text('Altura (cm)', style: Theme.of(context).textTheme.labelLarge),
                          const SizedBox(height: 4),
                          Text('$altura cm'),
                        ],
                        const SizedBox(height: 16),
                        if (modoEdicion)
                          TextFormField(controller: disponibilidadController, decoration: const InputDecoration(labelText: 'Días disponibles por semana (1–7)'))
                        else ...[
                          Text('Disponibilidad semanal', style: Theme.of(context).textTheme.labelLarge),
                          const SizedBox(height: 4),
                        ...
                        if (modoEdicion)
                          DropdownButtonFormField<String>(
                            ...
                            items: nivelesExperiencia.map((nivel) => DropdownMenuItem(value: nivel, child: Text(nivel))).toList(),
                            onChanged: (val) => setState(() => nivelSeleccionado = val),
                            decoration: const InputDecoration(labelText: 'Nivel de experiencia'),
                          )
                        else ...[
                          Text('Nivel de experiencia', style: Theme.of(context).textTheme.labelLarge),
                          const SizedBox(height: 4),
                          Text(nivel),
                        ],
                        const SizedBox(height: 16),
                        if (modoEdicion)
                          DropdownButtonFormField<String>(
                            ...
                            items: objetivos.map((obj) => DropdownMenuItem(value: obj, child: Text(obj))).toList(),
                            onChanged: (val) => setState(() => objetivoSeleccionado = val),
                          ...
                          child: modoEdicion
                              ? Row(
                                  ...
                                  children: [
                                    ...
                                    OutlinedButton.icon(
                                      onPressed: () => setState(() => modoEdicion = false),
                              ...
                              : ElevatedButton.icon(
                                  onPressed: () => setState(() => modoEdicion = true),
                        ...
                        if (incompleto)
                          Padding(
                            ...
                            child: ElevatedButton(
                              onPressed: () {
                                Navigator.push(context, MaterialPageRoute(builder: (_) => CompletarPerfilScreen(uid: uid!)));
                              },
                        ...
                        if (!incompleto)
                          Padding(
                            ...
                            child: ElevatedButton(
                              onPressed: () {
                                Navigator.push(context, MaterialPageRoute(builder: (_) => const MisRutinasScreen()));
                              },
  ...
  void _guardarCambios() async {
    if (_formKey.currentState!.validate()) {
      await FirebaseFirestore.instance.collection('usuarios').doc(FirebaseAuth.instance.currentUser!.uid).update({
        ...
        'disponibilidadSemanal': int.tryParse(disponibilidadController.text) ?? 0,
